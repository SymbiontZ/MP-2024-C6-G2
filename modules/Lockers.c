...
Vect_Lock Cargar_Lockers(){
	...
	char linea[MAX_LOCK];                                                                                                   // L�nea a leer
    int i = 0, m;
	...
	vector_lock.tam = 0;
	while(fgets(linea, sizeof(linea), f_lockers) != NULL)                                                                   // Contamos el n�mero de l�neas del fichero
	    vector_lock.tam++;
...
Vect_CompLock Cargar_CompartimentosLockers(){
	...
	char linea[MAX_COMPLOCK];                                                                                               // L�nea a leer
    int i = 0, m;
	...
	while(fgets(linea, sizeof(linea), f_compartimentoslockers) != NULL)                                                     // Contamos el n�mero de l�neas del fichero
	    vector_complock.tam++;
...
void Guardar_Lockers(Vect_Lock lockers){
	...
	char ruta[] = "..\\ESIZON-main\\data\\Lockers.txt";                                                                     // Ruta del fichero a leer
	char linea[MAX_LOCK];                                                                                                   // L�nea actual del fichero
	char aux[14];
...
void Guardar_CompartimentosLockers(Vect_CompLock compartimentoslockers){
	...
	char ruta[] = "..\\ESIZON-main\\data\\CompartimentosLockers.txt";                                                       // Ruta del fichero a leer
	char linea[MAX_COMPLOCK];                                                                                               // L�nea actual del fichero
	char aux[14];
...
Vect_Lock Alta_Lockers(Vect_Lock L){
    ...
    L.Lock = realloc(L.Lock, L.tam+1*sizeof(Vect_Lock));											// Se amplia la longitud del vector para a�adir un locker en la estructura
    if (L.Lock == NULL){
...
Vect_Lock Baja_Lockers(Vect_Lock L){
    ...
    for(i=0; i<L.tam; i++){
        ...
        if(strcmp(Id_locker_busqueda, L.Lock[i].Id_locker) == 0){                         //Una vez se encuentra una coincidencia de la id de busqueda en la estructura
            ...
            L.tam = L.tam - 1;                                                            //Se reduce el tama�o total del vector de estructuras
            break;
        }
    }
...
int Locker_Dispo(Vect_Lock L, int pos){
    ...
    if(L.Lock[i].Localidad == c.clients[pos].Localidad)                                                                         //Se busca una coincidencia de localidades
        ...
        printf("No existe ning�n locker disponible en tu localidad \n");
    ...
    for(i=0; i<L.tam; i++){
            ...
            if(strcmp(Id_locker_busqueda, L.Lock[i].Id_locker) == 0){                                                                       //Una vez se encuentra una coincidencia de la id de busqueda en la estructura
                    ...
                    do{
                        ...
                        strcpy(C.CompLock[cont].Estado, "vac�o");
...
Vect_CompLock Recogida_Locker(Vect_CompLock cl, int pos, int n_comp){
    ...
    for(i = 0; i<cl.tam; i++){
        if((strcpy(p.pedidos[pos].id_locker, cl.CompLock[i].Id_locker) == 0) && cl.CompLock[i].Num_comp == n_comp && fch == 1){     //Se comprueba que exista el compartimento deseado
            ...
            do{
                printf("Introduzca el c�digo de seguridad del locker: \n");                                                         //Se introduce el c�digo de seguridad
                scanf("%d", &Cod_locker_check);

                if(Cod_locker_check == cl.CompLock[i].Cod_locker){                                                                  //Se comprueba el c�digo de seguridad
                    printf("C�digo introducido correctamente \n");
                    printf("Se ha abierto el compartimento n�mero %d \n", &n_comp);
                    strcpy(cl.CompLock[i].Estado, "vac�o");

                }
                else{
                    printf("C�digo incorrecto, int�ntelo nuevamente: \n \n");
                }
                cont++;
            }while(cont < 3);

            if(cont == 3)
                printf("Se ha quedado sin intentos, cancelando operaci�n... \n");                                                   //En caso de fallar el c�digo 3 veces el sistema cancela la operaci�n
